    ...
    return(
        <section className={`transactionContainer ${styles.transactionStatusContainer} `}>
            <section className={styles.transactionStatusHeaderContainer}>
                ...
                <h3 className={`${styles.transactionStatusBox} ${status.confirmed? `${styles.transactionStatusConfirmedBox}`: `${styles.transactionStatusPendingBox}`}`}>{status.confirmed?'Confirmed':'Pending'}</h3>
            </section>
            <section className={styles.transactionIdSection}>
                {
                toggleCopyMessage?
                    <p ref={textRef} className={styles.transactionId}>Copied to Clipboard!</p>
                :
                    <>
                        <p ref={textRef} className={styles.transactionId}>{txid}</p>
            ...
            
            {
                ...
                <p className={styles.transactionStatusDescription}>
                    This transaction was first broadcasted on the Bitcoin network on <span className={styles.textHighlight}>{pendingTimeConversion(pendingTransactionTime[0])}.</span> The current value of this transaction (amount + fees) is now <span className={styles.textHighlight}>${addCommas(BTCtoDollars + FeestoDollars)}.</span>
                </p>
                :
                <p className={styles.transactionStatusDescription}>
                    This transaction was confirmed on <span className={styles.textHighlight}>{pendingTimeConversion(status.block_time)}.</span> The current value of this transaction (amount + fees) is now <span className={styles.textHighlight}>${addCommas(BTCtoDollars + FeestoDollars)}</span>
                </p>
            }
            ...
            <section className={`transactionStatusInnerContainer ${styles.transactionStatusInfoContainer}`}>
                <section className={styles.transactionStatusInfo}>
                    <div className={styles.statusInfoSection}>
                        ...
                        <p className="detailText"><span className={styles.textHighlight}>{BTCAmount}</span> BTC</p>
                        <p className="detailText">${addCommas(BTCtoDollars)}</p>
                    </div>
                    <div className={styles.statusInfoSection}>
                        <h5 className={`detailText ${styles.statusInfoHeader}`}>Fees</h5>
                        <p className="detailText"><span className={styles.textHighlight}>{addCommas(fee)}</span> SATS</p>
                ...
                <section className={styles.transactionStatusInfo}>
                    <div className={styles.statusInfoSection}>
                        ...
                        <p className="detailText"><span className={styles.textHighlight}>{vin.length} inputs</span></p>
                    </div>
                    <div className={styles.statusInfoSection}>
                        <h5 className={`detailText ${styles.statusInfoHeader}`}>To</h5>
                        <p className="detailText"><span className={styles.textHighlight}>{vout.length} outputs</span></p>
                    </div>
                </section>
            </section>
        </section>
